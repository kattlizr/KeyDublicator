...
emRWType getRWtype(){    
  ...
  if (answer == 0xFE){
    Serial.println(" Тип: Dallas RW-1990.1 ");
  ...
  if (answer == 0xFE){
    ...
    delay(10); pinMode(iButtonPin, INPUT);
    Serial.println(" Тип: Dallas RW-1990.2 ");
  ...
  if (OneWire::crc8(m1, 3) == answer) {
    ...
    Serial.println(" Тип: Dallas TM2004");
    ibutton.reset();
    return TM2004; // это Type: TM2004
  }
  ibutton.reset();
  Serial.println(" Тип: неизвестное семейство Dallas, пробуем TM-01! ");
...
bool searchIbutton(){
  ...
  if (addr[0] == 0x01) {                         // это ключ формата dallas
    ...
    if (OneWire::crc8(addr, 7) != addr[7]) {
      Serial.println("Неверная контрольная сумма (CRC)!");
      Sd_ErrorBeep();
  ...
  if ((addr[0]>>4) == 0x0E) Serial.println(" Тип: неизвестное семейство Dallas, возможно Cyfral в ключе Dallas.");
    else Serial.println(" Тип: неизвестное семейство Dallas.");
  keyType = keyUnknown;
  return true;
}
...
bool searchCyfral(){
  ...
  Serial.println(F(" Тип: Cyfral DC2000A "));
  return true;  
}
...
emRWType getRfidRWtype(){
  ...
  Serial.print(" Тип ключа: T5557. Код: ");
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // КОМАНДА 1: ВЫКЛЮЧИТЬ ПРОВЕРКУ ЧЁТНОСТИ
    if (bufString.equalsIgnoreCase("ignore")) {
      ignoreFilters = true;
      Serial.println(F("Фильтры чётности отключены."));
      clearLed();
      digitalWrite(G_Led, HIGH); delay(300); // Моргнем зеленым для подтверждения
      digitalWrite(G_Led, LOW); digitalWrite(B_Led, HIGH);
      return;
    }
    
    // КОМАНДА 2: ДЕФИНАЛИЗАЦИЯ (СБРОС КЛЮЧА)
    if (bufString.equalsIgnoreCase("reset")) {
      Serial.println(F("Запуск реанимации RW1990.1"));

      // Идеально правильный код вездехода с корректным CRC (2F)
      byte recoveryID[8] = { 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x2F }; 

      // 1. Снимаем запрет записи (открываем чип)
      ibutton.reset();
      ibutton.write(0xD1); // Команда флага записи для RW1990.1
      ibutton.write_bit(0); // 0 = разрешить запись для RW1990.1
      delay(10); 
      pinMode(iButtonPin, INPUT);

      // 2. Отправляем команду принудительной записи вслепую
      ibutton.reset();
      ibutton.write(0xD5); // Команда записи для RW1990.1
      
      // 3. Запись данных (для RW1990.1 данные пишутся ИНВЕРТИРОВАННО, как у вас в BurnByte)
      for (byte i = 0; i < 8; i++) {
        byte data = ~recoveryID[i]; // Инвертируем байт перед прошивкой
        for(byte n_bit=0; n_bit<8; n_bit++){ 
          ibutton.write_bit(data & 1);  
          delay(5); // Время на фиксацию бита чипом
          data = data >> 1;
        }
      }
      delay(20);
      ...
      // 4. Закрываем запись (возвращаем флаг)
      ibutton.reset();
      ibutton.write(0xD1);
      ibutton.write_bit(1); // 1 = запретить запись
      delay(10);
      pinMode(iButtonPin, INPUT);

      // Сброс флагов дубликатора
      readflag = false;
      writeflag = false;
      ignoreFilters = false; 
      clearLed();
      ...
      Serial.println(F("Ключ реанимирован!"));
      Sd_ReadOK(); 
      return; 
    }
    
    // РАЗДЕЛЯЕМ СТРОКУ НА КОД И ТИП КЛЮЧЕЙ (ПО ПРОБЕЛУ)
    int spaceIndex = bufString.indexOf(' ');
    String hexString = bufString;
    String typeString = ""; 
    
    if (spaceIndex != -1) {
      hexString = bufString.substring(0, spaceIndex);
      typeString = bufString.substring(spaceIndex + 1);
      typeString.trim();
    ...
    if (hexString.length() == 16) {
      for (byte i = 0; i < 8; i++) {
        ...
        keyID[i] = (byte) strtol(byteString.c_str(), NULL, 16);
        addr[i] = keyID[i];       
      }
      
      if (typeString.length() > 0) {
        if (typeString == "cyfral") {
          keyType = keyCyfral;
          Serial.println(F("Тип выбран вручную: CYFRAL (С финализацией!)"));
        } 
        else if (typeString == "metakom") {
          keyType = keyMetacom;
          Serial.println(F("Тип выбран вручную: METAKOM (С финализацией!)"));
        } 
        else {
          keyType = keyDallas;
          Serial.println(F("Тип выбран вручную: Dallas (Без финализации!)"));
        }
      } else {
        // Умный автовыбор (если через пробел ничего не написали)
        if (keyID[0] == 0x01) {
          keyType = keyDallas;
          Serial.println(F("Авто-выбор: Dallas (01 в начале, без финалзации)"));
        } 
        else {
          keyType = keyDallas; 
          Serial.println(F("Авто-выбор: Dallas (Защита от финализации! Для финализации добавьте 'cyfral' или 'metacom')"));
        }
      }
      ...
      Serial.print(F("Ключ принят:"));
      for (byte i = 0; i < 8; i++) {
        Serial.print(keyID[i], HEX);
        if (i < 7) Serial.print(":");
      }
      Serial.println(F("Режим: запись"));
      Sd_ReadOK(); 
    } else {
      Serial.println(F("-> Ошибка! Формат: [КОД] или [КОД тип]. Пример:\n-> 01:A2:B3:C4:D5:E6:F7:88\n-> 00:00:FF:FF:FF:FF:00:00 metakom/cyfral/dallas"));
    }
  }
  ...
  if (BtnClick) {  
    if (readflag == true) {
      ...
      if (writeflag) digitalWrite(R_Led, HIGH);
          ...
          ignoreFilters = false; // Принудительно включаем фильтры обратно при выходе в режим чтения
          Serial.println(F("Фильтр чётности включен обратно!"));
        }
        
      Serial.print("WriteFlag = "); Serial.print(writeflag); Serial.print(" | KeyID = <");
      for (byte i = 0; i < 8; i++) { 
        Serial.print(keyID[i], HEX);
        Serial.print(":"); 
      }; 
  ...
  // === 3. РЕЖИМ ОБЫЧНОГО ДУБЛИРОВАНИЯ (С УЧЕТОМ ТУМБЛЕРА IGNORE) ===
  if (!writeflag){
    if (searchCyfral() || searchMetacom() || searchEM_Marine() || searchIbutton()){ 
      ...
      // Выводим в порт, какой именно протокол сейчас определила автоматика дубликатора
      Serial.print(F("Ключ считан. Тип: "));
      if (keyType == keyDallas) Serial.println(F("Dallas"));
      else if (keyType == keyCyfral) Serial.println(F("CYFRAL"));
      else if (keyType == keyMetacom) Serial.println(F("METAKOM"));
      else if (keyType == keyEM_Marie) Serial.println(F("EM-Marine"));
      
      ignoreFilters = false; // Сбрасываем режим игнорирования после успешного чтения
    } else {
      delay(100);   
      return;
    } 
